...
public class TypedParameter<T> {
    ...
    private final ClickHouseDataType chType;
    ...
    public TypedParameter(Class<T> type, ClickHouseDataType chType, String name, T defaultValue) {
    ...
    public TypedParameter(Class<T> type, ClickHouseDataType chType, String name, T defaultValue, T value) {
        this.type = Objects.requireNonNull(type);

        if (!type.isPrimitive() && type != BigDecimal.class && type != Double.class && type != Float.class
        ...
        if (chType != null) {
            ...
            if (defaultValue instanceof BigDecimal) {
                this.chType = ClickHouseDataType.Decimal;
            } else if (defaultValue instanceof Float) {
                this.chType = ClickHouseDataType.Float32;
            } else if (defaultValue instanceof Double) {
                this.chType = ClickHouseDataType.Float64;
            } else if (defaultValue instanceof Long) {
                this.chType = ClickHouseDataType.UInt64;
            } else if (defaultValue instanceof Number) {
                this.chType = ClickHouseDataType.Int32;
            } else {
                this.chType = ClickHouseDataType.String;
            }
        }
    ...
    public TypedParameter<T> merge(JsonObject p) {
        if (p != null) {
            ...
            if (this.type.isAssignableFrom(BigDecimal.class)) {
                ...
                newValue = (T) p.getDouble(this.name);
            } else if (this.type.isAssignableFrom(Float.class)) {
                newValue = (T) p.getFloat(this.name);
            } else if (this.type.isAssignableFrom(Long.class)) {
                newValue = (T) p.getLong(this.name);
            } else if (this.type.isAssignableFrom(Boolean.class)) {
                newValue = (T) p.getBoolean(this.name);
            } else if (this.defaultValue instanceof Number) {
                newValue = (T) p.getInteger(this.name);
            } else {
                newValue = (T) p.getString(this.name);
            }
    ...
    public TypedParameter<T> writeValueTo(ClickHouseBuffer buffer, int precision, int scale, TimeZone timezone) {
        switch (this.chType) {
            case Int8:
